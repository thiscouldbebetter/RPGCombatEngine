...
class DemoData
{
	universe()
	{
		...
		var actionDefnsCommonByName =
			ArrayHelper.addLookupsByName(actionDefnsCommon);

		var agentDefns =
			this.universe_3_AgentDefns(actionDefnsCommonByName);
		...
		var encounterDefnsByName =
			ArrayHelper.addLookupsByName(encounterDefns);

		var encounter = this.universe_4_Encounter
		(
			encounterDefnsByName, agentDefnsByName, itemDefnsByName
		);
	...
	universe_1_ItemDefns(categoriesByName: Map<string, Category>)
	{
		var itemDefns =
		[
			// consumables
			...
			(
				"Heal Potion",
				[ categoriesByName.get("Consumable").name ],
				(item, agent, target) => // apply
				{
			...
			(
				"Coin",
				[ categoriesByName.get("Valuable").name ],
				(item, agent, target) => // apply
				{
					// Do nothing.
				}
			...
			(
				"Club",
				[ categoriesByName.get("Weapon").name ],
				(item, agent, target) => // apply
				{
					target.integrity -= 5;
				}
		...
		[
			...
			(
				...
				(encounter, agent, action) => // perform
				{
					...
					{
						var emptyToReturnTo = action.parameterByName
						(
							"EmptyForPosToReturnTo"
						);
						...
						{
							var weaponEquipped =
								agent.itemsEquippedByName.get("Weapon");
			...
			(
				...
				(encounter, agent, action) => // perform
				{
					...
					{
						var emptyToReturnTo =
							action.parameterByName("EmptyForPosToReturnTo");
						...
						{
							var spellDefn =
								action.parameterByName("SpellDefn");
			...
			(
				...
				(encounter, agent, action) => // perform
				{
					...
					else
					{
						...
						{
							var itemEquipped =
								agent.itemsEquippedByName.get("ItemToUse");
	...
	universe_3_AgentDefns(actionDefnsCommonByName: Map<string, ActionDefn>)
	{
		var agentSizeInPixelsStandard = Coords.fromXY(24, 24);

		var visualAgentLabel = new VisualDynamic
		(
			(universe, world, display, entity) => // draw
			{
				var agent = entity;
	...
	universe_4_Encounter
	(
		encounterDefnsByName: Map<string, EncounterDefn>,
		agentDefnsByName: Map<string, AgentDefn>,
		itemDefnsByName: Map<string, ItemDefn>
	)
	{
		var encounterDefnDefaultName =
			encounterDefnsByName.get("Default").name;

		var encounter = new Encounter
		(
			encounterDefnDefaultName,
			// parties
			[
				new Party
				(
					...
					[
						...
						(
							"One",
							agentDefnsByName.get("Mage").name,
							Coords.fromXY(256, 20),
							// itemsEquipped
							[
						...
						(
							"Two",
							agentDefnsByName.get("Priest").name,
							Coords.fromXY(256, 50),
							// itemsEquipped
							[
						...
						(
							"Three",
							agentDefnsByName.get("Warrior").name,
							Coords.fromXY(256, 80),
							// itemsEquipped
							[
						...
						(
							"Four",
							agentDefnsByName.get("Rogue").name,
							Coords.fromXY(256, 110),
							// itemsEquipped
							[
								new Item(itemDefnsByName.get("Dagger").name)
							],
				...
				(
					...
					[
						...
						(
							null, // name
							agentDefnsByName.get("Troll").name,
							Coords.fromXY(90, 20),
							[ new Item(itemDefnsByName.get("Club").name) ], // itemsEquipped
							[ new Item(itemDefnsByName.get("Coin").name, 5) ] // itemsInInventory
						),
						new Agent
						(
							null, // name
							agentDefnsByName.get("Goblin").name,
							Coords.fromXY(30, 50),
							[ new Item(itemDefnsByName.get("Dagger").name) ], // itemsEquipped
