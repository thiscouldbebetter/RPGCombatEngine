"use strict";
class Menu {
    constructor(text, pos, spacing, menuable, updateEncounter, children, indexOfChildSelected) {
        this.text = text;
        this.pos = pos;
    ...
    static menuablesToMenus(menuables, bindingPathsForMenuText, updateEncounter) {
        ...
        for (var i = 0; i < menuables.length; i++) {
            ...
            if (menuable.toMenu != null) {
                ...
                menuableAsMenu = new Menu(menuText, new Coords(0, 0), // pos
                new Coords(0, 8), // spacing
                menuable, // menuable
                updateEncounter, null // children
                );
            }
    ...
    updateEncounterForTimerTick(encounter) {
        ...
        if (inputHelper.hasKeyPressedBeenProcessed == false) {
            ...
            if (keyPressed == "ArrowLeft") {
                ...
                var hasNoChildren = (childSelected.children == null || childSelected.children.length == 0);
                if (hasNoChildren) {
    ...
    draw(universe, world, display) {
        ...
        var drawPos = pos.clone().add(new Coords(arrowSize.x + 2, spacing.y / 2));
