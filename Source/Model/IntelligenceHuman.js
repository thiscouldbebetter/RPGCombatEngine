...
class IntelligenceHuman {
    actionInitialize(action, encounter, agent) {
        ...
        var menuForActionDefns = new Menu("Actions", paneMenuPlayer.pos, // pos
        new Coords(0, 8), // spacing
        null, // menuable
        null, // updateEncounter
        Menu.menuablesToMenus(actionDefns, ["name"], // bindingPathsForMenuText
        updateEncounter), 0 // indexOfChildSelected
        );
    ...
    decideActionTarget(action, encounter, agent) {
        ...
        if (target == null) {
            ...
            action.parameterByNameSet("EmptyForPosToReturnTo", new Empty(agent.pos.clone()));
        }
